...
type ThumbnailJobEnsurer interface {
	EnsureThumbnailJobTx(context.Context, pgx.Tx, uuid.UUID, string, time.Time) (bool, error)
}
...
func (s *Service) bindUploads(ctx context.Context, tx pgx.Tx, ownerID, messageID uuid.UUID, uploadIDs []uuid.UUID, startOrder int, now time.Time) (bool, error) {
	if len(uploadIDs) == 0 {
		return false, nil
	}
	...
	rows, err := tx.Query(ctx, `SELECT us.id,us.file_object_id,us.original_filename,us.client_mime,us.status,us.consumed_at,fo.status,fo.detected_mime FROM upload_sessions us JOIN file_objects fo ON fo.id=us.file_object_id WHERE us.user_id=$1 AND us.id=ANY($2::uuid[]) ORDER BY us.id FOR UPDATE OF us,fo`, ownerID, sorted)
	if err != nil {
		return false, err
	}
	...
	for rows.Next() {
		...
		if err = rows.Scan(&u.ID, &u.FileID, &u.Filename, &u.ClientMime, &status, &consumed, &fileStatus, &u.DetectedMime); err != nil {
			return false, err
		}
		if consumed != nil {
			return false, ErrUploadAlreadyConsumed
		}
	...
	for order, uploadID := range uploadIDs {
		...
		if s.thumbnailJobs != nil {
			created, ensureErr := s.thumbnailJobs.EnsureThumbnailJobTx(ctx, tx, u.FileID, u.DetectedMime, now)
			if ensureErr != nil {
...
func (s *Service) copyAttachments(ctx context.Context, tx pgx.Tx, sourceID, destinationID uuid.UUID, now time.Time) (bool, error) {
	var total int
	if err := tx.QueryRow(ctx, `SELECT count(*) FROM message_attachments WHERE message_id=$1`, sourceID).Scan(&total); err != nil {
		return false, err
	}
	rows, err := tx.Query(ctx, `SELECT ma.file_object_id,ma.original_filename,ma.client_mime,ma.display_order,fo.detected_mime FROM message_attachments ma JOIN file_objects fo ON fo.id=ma.file_object_id WHERE ma.message_id=$1 AND fo.status='READY' ORDER BY ma.display_order,ma.id`, sourceID)
	if err != nil {
		return false, err
	}
	...
	for rows.Next() {
		...
		if err = rows.Scan(&item.fileID, &item.name, &item.mime, &item.order, &item.detectedMime); err != nil {
			rows.Close()
			return false, err
		}
	...
	for _, item := range items {
		...
		if s.thumbnailJobs != nil {
			created, ensureErr := s.thumbnailJobs.EnsureThumbnailJobTx(ctx, tx, item.fileID, item.detectedMime, now)
			if ensureErr != nil {
				return false, ensureErr
			}
			jobCreated = jobCreated || created
...
func (s *Service) AddAttachments(ctx context.Context, ownerID, messageID uuid.UUID, expected int64, uploadIDs []uuid.UUID) (Message, error) {
	...
	err := s.repo.withTx(ctx, func(tx pgx.Tx) error {
		...
		var created bool
		if created, err = s.bindUploads(ctx, tx, ownerID, messageID, uploadIDs, start, now); err != nil {
			return err
		}
		jobCreated = jobCreated || created
		m.Version++

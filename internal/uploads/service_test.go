...
func newMemoryRepo() *memoryRepo {
	return &memoryRepo{sessions: map[uuid.UUID]Session{}, parts: map[uuid.UUID]map[int]Part{}, settings: Settings{MaxFileSizeBytes: 1 << 30, UploadRetentionHours: 24}}
}
...
func (r *memoryRepo) FindExpired(_ context.Context, now time.Time, batch int32) ([]uuid.UUID, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	out := []uuid.UUID{}
	for id, s := range r.sessions {
		...
		if s.Status == Expired || ((s.Status == Created || s.Status == Uploading || s.Status == Failed) && !s.ExpiresAt.After(now)) {
			out = append(out, id)
		}
	}
	return out, nil
...
func (r *memoryRepo) ActiveExists(_ context.Context, id uuid.UUID) (bool, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	s, ok := r.sessions[id]
	return ok && (s.Status == Created || s.Status == Uploading || s.Status == Completing || s.Status == Failed), nil
}
...
func newMemoryStaging() *memoryStaging { return &memoryStaging{files: map[uuid.UUID][]byte{}} }

  ...
  <Teleport to="body">
    ...
    >
      ...
      >
        <header>
          <div>
            <span class="eyebrow">传输队列</span><h2 id="queue-title">
              上传任务
            </h2>
          </div><button
            class="button close"
...
<style scoped>
.queue-backdrop{position:fixed;inset:0;z-index:70;background:rgb(16 24 21 / .28);display:flex;justify-content:flex-end}.queue{width:min(430px,94vw);height:100%;overflow:auto;border-radius:var(--radius-lg) 0 0 var(--radius-lg);padding:1rem 1.15rem;box-shadow:var(--shadow)}header{display:flex;align-items:flex-start;justify-content:space-between;border-bottom:1px solid var(--border);padding-bottom:.8rem;margin-bottom:.3rem}h2{margin:.15rem 0;font-size:1.15rem}.eyebrow{font:700 .67rem/1 var(--font-mono);letter-spacing:.13em;text-transform:uppercase;color:var(--accent-strong)}.close svg{width:1rem;height:1rem}.empty{padding:2.2rem .5rem;text-align:center;color:var(--muted)}.warning{margin:.5rem 0 0;padding:.55rem .65rem;border-radius:var(--radius-sm);background:var(--surface-soft);color:#9a6414;font-size:.76rem;line-height:1.5}.privacy{font-size:.72rem;line-height:1.5;color:var(--muted);border-top:1px solid var(--border);padding-top:.8rem}

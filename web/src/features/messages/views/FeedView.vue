...
const title = computed(() => ({ temporary:'Temporary', permanent:'Permanent', favorites:'收藏', tag: tags.data.value?.find((tag) => tag.id === props.tagId)?.name || '标签', trash:'回收站' })[props.kind])

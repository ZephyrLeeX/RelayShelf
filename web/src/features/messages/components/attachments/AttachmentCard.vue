...
<template>
  ...
  >
    ...
    <a
      ...
      @click.stop
    ><Download aria-hidden="true" />下载</a>
  </div>
</template>
...
.download-button{display:inline-flex;align-items:center;justify-content:center;gap:.3rem;min-height:30px;border:1px solid var(--border-default);border-radius:var(--radius-sm);padding:.25rem .5rem;background:var(--surface-raised);color:var(--text-secondary);font-size:.72rem;font-weight:650;text-decoration:none}.download-button svg{width:.8rem;height:.8rem}.download-button:hover{border-color:var(--border-strong);color:var(--accent-primary)}

...
async function render(overrides = {}, trash = false) {
  ...
  return mount(MessageCard, { props: { message: messageFixture(overrides), trash }, global: { plugins: [router, [VueQueryPlugin, { queryClient: new QueryClient() }]] } })
}
...
describe('MessageCard', () => {
  ...
  it('hides favorite for temporary messages and offers it for permanent messages', async () => {
    expect((await render({ lifecycle: Lifecycle.TEMPORARY })).text()).not.toContain('收藏')
    expect((await render({ lifecycle: Lifecycle.PERMANENT })).text()).toContain('收藏')
  })

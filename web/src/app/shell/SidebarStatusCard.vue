...
<template>
  ...
  >
    ...
    >
      {{ active ? '正在中继' : uploadCount ? `${uploadCount} 个上传任务` : '上传队列为空' }}<b aria-hidden="true">↗</b>
    </button>
  </section>
</template>
...
.status-head{display:flex;align-items:center;gap:.5rem;font-size:.78rem}.status-head i{width:.55rem;height:.55rem;border-radius:50%;background:var(--state-warning)}.status-head i.connected{background:var(--accent-secondary);box-shadow:0 0 0 4px color-mix(in srgb,var(--accent-secondary) 16%,transparent)}.device,.storage{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--text-secondary);font-size:.7rem}.storage[data-state='DEGRADED'],.storage[data-state='UNAVAILABLE']{color:var(--state-warning)}.queue{display:flex;justify-content:space-between;width:100%;padding:.25rem 0 0;border:0;border-top:1px solid color-mix(in srgb,var(--border-default) 65%,transparent);background:transparent;color:var(--text-tertiary);font-size:.68rem;text-align:left}.queue b{color:var(--accent-secondary)}
</style>
